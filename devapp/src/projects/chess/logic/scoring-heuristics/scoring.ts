import ChessBoard from "../game/board"
import { Color } from '../../models/Piece'

export function calculatePiecesScore(board: ChessBoard, color: Color) {
    let score = 0
    const piecesByType = board._data._bitboards[color]
    for(let i = 0; i < 64; i++) {
        const flag = board.getFlag(i)
        if (board.hasPiece(piecesByType['P'], flag)) score += 1;
        else if (board.hasPiece(piecesByType['N'], flag)) score += 3;
        else if (board.hasPiece(piecesByType['B'], flag)) score += 3;
        else if (board.hasPiece(piecesByType['R'], flag)) score += 5;
        else if (board.hasPiece(piecesByType['Q'], flag)) score += 9;
    }
    return score
}
export function calculatePositionalScore(board: ChessBoard, color: Color) {
    const pieces = board.getPiecesForColor(color)
    let positionalScore = 0;
    for(let i = 0; i < 64; i++) {
        const flag = board.getFlag(i)
        if (board.hasPiece(pieces, flag)) {
            const piece = board.getPiece(flag) 
            if (piece == null) throw new Error('piece should never be null here')
            const index = color === 'white' ? 63 - i : i
            const score = squareTable[piece][index]
            positionalScore += score;
        }
    }
    return positionalScore
}

const POSITION_WEIGTH = 0.03

export function calculateScoreOfColor(board: ChessBoard, color: Color) {
    let scoreOfColor = calculatePiecesScore(board, color)
    scoreOfColor += (calculatePositionalScore(board, color) * POSITION_WEIGTH)
    return scoreOfColor
}

export function calculateScoreDelta(color: Color, board: ChessBoard) {
    if (color === 'white') return calculateScoreOfColor(board, 'white') - calculateScoreOfColor(board, 'black')
    return calculateScoreOfColor(board, 'black') - calculateScoreOfColor(board, 'white')
}

export function calculateOverallScoreDelta(board: ChessBoard) {
    return calculateScoreOfColor(board, 'white') - calculateScoreOfColor(board, 'black')
}


const squareTable: { [key: string] : number[] } = {
    'P': [
            0, 0, 0, 0, 0, 0, 0, 0,
            50, 50, 50, 50, 50, 50, 50, 50,
            10, 10, 20, 30, 30, 20, 10, 10,
            5, 5, 10, 25, 25, 10, 5, 5,
            0, 0, 0, 20, 20, 0, 0, 0,
            5, -5, -10, 0, 0, -10, -5, 5,
            5, 10, 10, -20, -20, 10, 10, 5,
            0, 0, 0, 0, 0, 0, 0, 0
    ],
    'N':  [
            -50, -40, -30, -30, -30, -30, -40, -50,
            -40, -20, 0, 0, 0, 0, -20, -40,
            -30, 0, 10, 15, 15, 10, 0, -30,
            -30, 5, 15, 20, 20, 15, 5, -30,
            -30, 0, 15, 20, 20, 15, 0, -30,
            -30, 5, 10, 15, 15, 10, 5, -30,
            -40, -20, 0, 5, 5, 0, -20, -40,
            -50, -40, -30, -30, -30, -30, -40, -50
    ],
    'B': [
            -20, -10, -10, -10, -10, -10, -10, -20,
            -10, 0, 0, 0, 0, 0, 0, -10,
            -10, 0, 5, 10, 10, 5, 0, -10,
            -10, 5, 5, 10, 10, 5, 5, -10,
            -10, 0, 10, 10, 10, 10, 0, -10,
            -10, 10, 10, 10, 10, 10, 10, -10,
            -10, 5, 0, 0, 0, 0, 5, -10,
            -20, -10, -10, -10, -10, -10, -10, -20
    ],
    'R': [
            0, 0, 0, 5, 5, 0, 0, 0,
            -5, 0, 0, 0, 0, 0, 0, -5,
            -5, 0, 0, 0, 0, 0, 0, -5,
            -5, 0, 0, 0, 0, 0, 0, -5,
            -5, 0, 0, 0, 0, 0, 0, -5,
            -5, 0, 0, 0, 0, 0, 0, -5,
            5, 10, 10, 10, 10, 10, 10, 5,
            0, 0, 0, 0, 0, 0, 0, 0
    ],
    'Q': [
            -20, -10, -10, -5, -5, -10, -10, -20,
            -10, 0, 0, 0, 0, 0, 0, -10,
            -10, 5, 5, 5, 5, 5, 0, -10,
            0, 0, 5, 5, 5, 5, 0, -5,
            -5, 0, 5, 5, 5, 5, 0, -5,
            -10, 0, 5, 5, 5, 5, 0, -10,
            -10, 0, 0, 0, 0, 0, 0, -10,
            -20, -10, -10, -5, -5, -10, -10, -20
    ],
    'K': [
            -30, -40, -40, -50, -50, -40, -40, -30,
            -30, -40, -40, -50, -50, -40, -40, -30,
            -30, -40, -40, -50, -50, -40, -40, -30,
            -30, -40, -40, -50, -50, -40, -40, -30,
            -20, -30, -30, -40, -40, -30, -30, -20,
            -10, -20, -20, -20, -20, -20, -20, -10,
            20, 20, 0, 0, 0, 0, 20, 20,
            20, 30, 10, 0, 0, 10, 30, 20
    ], 'K_endgame': [ // King (Endgame)
        -50, -30, -30, -30, -30, -30, -30, -50,
        -30, -30, 0, 0, 0, 0, -30, -30,
        -30, -10, 20, 30, 30, 20, -10, -30,
        -30, -10, 30, 40, 40, 30, -10, -30,
        -30, -10, 30, 40, 40, 30, -10, -30,
        -30, -10, 20, 30, 30, 20, -10, -30,
        -30, -20, -10, 0, 0, -10, -20, -30,
        -50, -40, -30, -20, -20, -30, -40, -50
    ]
};